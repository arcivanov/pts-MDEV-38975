...
#   SAR_OUTPUT       — if set, record SAR data to this file (e.g. ~/sar-baseline.dat)
#
...
if [ -n "$SAR_OUTPUT" ]; then
    sar -u -r -b -B 1 -o "$SAR_OUTPUT" > /dev/null 2>&1 &
    SAR_PID=$!
    echo "SAR recording: $SAR_OUTPUT (PID $SAR_PID)"
    echo ""
fi
...
if [ -n "$SAR_PID" ]; then
    ...
    echo "SAR data saved to: $SAR_OUTPUT"
fi
